            ...
            if _PADDLE_OCR_INSTANCE is None:
                try:
                    ...
                    _PADDLE_OCR_INSTANCE = PaddleOCR(
                        use_angle_cls=False,  # We handle rotation manually
                        lang='en',
                        use_gpu=False,
                        show_log=False,
                        det=True,
                        rec=True,
                        cls=False,
                        rec_batch_num=1,  # Faster for single images
                        det_db_thresh=0.3,
                        det_db_box_thresh=0.5,
                        use_mp=False,  # Disable internal multiprocessing
                        total_process_num=1,
                        cpu_threads=12,  # ✅ Use 4 CPU threads
                    )
...
def ocr_coordinate_strong(det: dict, bgr_color: np.ndarray, engine: str) -> str:
    ...
    if not _is_cardinal(ang_normalized):
        # ✅ GENTLER line removal - don't destroy brackets
        pil_pass1 = _remove_long_lines_oriented(pil_crop, ang_raw, frac_len=0.30, thickness=2)
        pil_pass2 = _remove_long_lines_oriented(pil_pass1, ang_raw, frac_len=0.25, thickness=6)
        pil_final = _remove_long_lines_oriented(pil_pass2, ang_raw, frac_len=0.20, thickness=10)
        
        if DEBUG_ANGLE_ROUTING:
            print(f"   → Applied 3-pass line removal (gentle)")
    else:
        pil_final = pil_crop
        if DEBUG_ANGLE_ROUTING:
