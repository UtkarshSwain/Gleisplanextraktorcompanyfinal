...
def init_db():
    ...
    try:
        ...
        # Add new columns if they don't exist (migration for older databases)
        cursor = conn.cursor()
        ...
        if result:
            ...
            if learned_patterns_json:
                try:
                    ...
                    print(f"  Could not parse learned patterns: {e}")
            ...
            if uncertain_detections_json:
                try:
                    ...
                    print(f"  Could not parse uncertain detections: {e}")
...
def save_workspace_data(layout_name: str, workspace_data: List[Dict],
    ...
    """
    Saves (Inserts or Updates) the workspace data, track skeleton, image dimensions, and metadata.
    ...
    Args:
        ...
        learned_patterns: Optional OCR learning patterns dictionary
        uncertain_detections: Optional list of low-confidence detection items
    """
    ...
    try:
        ...
        """, (layout_id, data_as_json_string, track_skeleton_compressed, image_dimensions_json,
              learned_patterns_json, uncertain_detections_json, datetime.now().isoformat()))
