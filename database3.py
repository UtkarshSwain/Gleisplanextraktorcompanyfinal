...
def init_db():
    ...
    # ✅ NEW: Manual corrections log table
    ...
    # ✅ NEW: Correction statistics view
    sql_create_correction_stats_view = """
    ...
    # Add track_skeleton column if it doesn't exist
    ...
    try:
        with db_cursor(commit=True) as cursor:
            ...
            # ✅ NEW: Create manual corrections table
            cursor.execute(sql_create_manual_corrections)
            cursor.execute(sql_create_correction_stats_view)
            
            # ✅ Create indexes
            cursor.execute(sql_create_indexes)
            
            print("✅ Database tables are ready (including validation and correction tracking).")
...
def save_validation_results(layout_name: str, validation_results: List):
    ...
    # Clear old validation results for this layout
    sql_clear_old = "DELETE FROM validation_log WHERE layout_id = %s;"
    ...
    with db_cursor(commit=True) as cursor:
        ...
        if not layout_id_result:
            print(f"⚠️ Layout {layout_name} not found in database")
            return
        ...
        for result in validation_results:
            # Extract validation type from message
...
def get_quality_metrics(layout_name: str, metric_name: Optional[str] = None) -> List[Dict]:
    ...
    if metric_name:
        ...
        WHERE t.layout_name = %s AND q.metric_name = %s
...
# ============================================================================
# ✅ NEW: MANUAL CORRECTION FUNCTIONS
# ============================================================================
...
def get_correction_history(layout_name: str, row_id: Optional[int] = None) -> List[Dict]:
    ...
    if row_id is not None:
        ...
        FROM manual_corrections mc
        JOIN track_layouts t ON mc.layout_id = t.id
        WHERE t.layout_name = %s AND mc.row_id = %s
